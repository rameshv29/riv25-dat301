...
# Add CloudWatch-specific health check endpoint
@mcp.custom_route("/cloudwatch/health", methods=["GET"])
async def cloudwatch_health_check(request: Request) -> PlainTextResponse:
    """CloudWatch MCP server health check endpoint."""
    return PlainTextResponse("CloudWatch MCP Server - OK")


def main():
    """Run the MCP server."""
    # Configure the MCP server settings for container deployment
